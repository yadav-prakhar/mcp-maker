...
export async function validateMCPProject(): Promise<void> {
  ...
  if (!await fs.pathExists(serverDirPath)) {
    throw new Error(chalk.red('Error: Not in an MCP server project. No src/server directory found.'));
  }
  
  // Check if src/tools directory exists
  const toolsDirPath = path.join(cwd, 'src', 'tools');
  if (!await fs.pathExists(toolsDirPath)) {
    throw new Error(chalk.red('Error: Not in an MCP server project. No src/tools directory found.'));
  }
  ...
  if (!await fs.pathExists(servicesDirPath)) {
    throw new Error(chalk.red('Error: Not in an MCP server project. No src/services directory found.'));
  }
}