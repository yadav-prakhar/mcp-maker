...
export async function addService(name?: string): Promise<void> {
	try {
			...
			const response = await prompts([
				{
					...
					validate: (value: string) =>
						/^[a-z0-9-]+$/.test(value) ? true : "Service name can only contain lowercase letters, numbers, and hyphens",
				},
			]);
		...
		await fs.writeFile(path.join(serviceDir, `${fileName}.ts`), compiledServiceTemplate(templateData));
