 ...
 */
export async function createServer(name?: string, options?: { http?: boolean; cors?: boolean; port?: number; install?: boolean }): Promise<void> {
		...
		const response = await prompts([
			{
				...
				validate: (value: string) =>
					/^[a-z0-9-]+$/.test(value) ? true : "Project name can only contain lowercase letters, numbers, and hyphens",
			},
		]);
