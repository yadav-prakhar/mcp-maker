			...
			const response = await prompts([
				{
					...
					validate: (value: string) =>
						/^[a-z0-9-]+$/.test(value) ? true : "Tool name can only contain lowercase letters, numbers, and hyphens",
				},
			]);
		...
		if (await fs.pathExists(mainToolsIndexPath)) {
			...
			if (!mainToolsIndex.includes(importStatement)) {
				...
				if (matches.length > 0) {
					...
					mainToolsIndex = mainToolsIndex.substring(0, position) + importStatement + "\n" + mainToolsIndex.substring(position);
				} else {
			...
			if (!mainToolsIndex.includes(exportStatement)) {
				...
				if (exportMatch) {
					...
					if (toolsArrayMatch) {
						...
						mainToolsIndex = mainToolsIndex.substring(0, position) + exportStatement + "\n\n" + mainToolsIndex.substring(position);
					} else {
			...
			if (!mainToolsIndex.includes(toolsArrayItem)) {
				...
				if (toolsArrayMatch) {
					...
					mainToolsIndex = mainToolsIndex.substring(0, position) + "\n  " + toolsArrayItem + mainToolsIndex.substring(position);
				} else {
...
export const serverTools: Tool[] = [
  ...${camelName}Tools,
		...
		if (await fs.pathExists(toolHandlerPath)) {
			...
			if (!toolHandler.includes(importStatement)) {
				...
				if (matches.length > 0) {
					...
					// Insert the new import after the last import
					toolHandler = toolHandler.substring(0, position) + importStatement + "\n" + toolHandler.substring(position);
				} else {
					// No imports found, add at the beginning
        ...
        `;
						...
						toolHandler = toolHandler.substring(0, position) + caseBlock + toolHandler.substring(position);
					}
				}
			}
